...
const UndoRedo = ({ canUndo, canRedo, onUndo, onRedo }) => (
    <div style={{ display: 'grid' }}>
        <button onClick={onUndo} disabled={!canUndo} style={{ padding: '5px', margin: '5px auto', color: !canUndo ? 'lightgray' : 'black', width: '200px', fontSize: '20px' }}>
            <i className="material-icons">
                undo
            </i>
        </button>
        <button onClick={onRedo} disabled={!canRedo} style={{ padding: '5px', margin: '5px auto', color: !canRedo ? 'lightgray' : 'black', width: '200px', fontSize: '20px' }}>
            <i className="material-icons">
                redo
            </i>
        </button>
